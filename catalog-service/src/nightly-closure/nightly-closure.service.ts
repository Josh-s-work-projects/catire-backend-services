...
export class NightlyClosureService {
  ...
  // Obtener inventario teórico para cierre
  async getTheoreticalStock(branchId: number, date: string) {
    ...
    return ingredients.map(ing => ({
      ...
      theoretical: previousMap.get(ing.id) || ing.stock, // Se calculará con las ventas del día
    }));
  }

  // Guardar cierre de noche
  async saveClosure(branchId: number, date: string, closures: any[], closedBy: number) {
    ...
    for (const closure of closures) {
      ...
      // Actualizar el stock del ingrediente al valor físico real
  ...
  // Obtener apertura del día (último cierre)
